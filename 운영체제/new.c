...
void print_result()
{
    int LSD_freq_sum = 0, MSD_freq_sum = 0;

    for (int i = 0; i < 10; i++)
    {
        LSD_freq_sum += LSD_freq[i];
        MSD_freq_sum += MSD_freq[i];
    }

    printf("   |  십진수의 MSD 숫자 빈도 및 확률(0~9)    |  십진수의 LSD 숫자 빈도 및 확률(0~9)\n");
    printf("ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ\n");

    for (int i = 0; i < 10; i++)
    {
        printf(" %d |    %10d(약 %10lf%%)  %10c  %10d(약 %10lf%%)                  \n",
            i, MSD_freq[i], ((double)MSD_freq[i] / MSD_freq_sum) * 100,'|', LSD_freq[i], ((double)LSD_freq[i] / LSD_freq_sum) * 100);
    }
